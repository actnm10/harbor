...
let activeWordPreviews = 0;

export async function createTextPreview({ filePath, name, size }) {
  const extension = path.extname(name).toLowerCase();
  const isWord = extension === '.doc' || extension === '.docx';
  if (!isWord && !textExtensions.has(extension)) throw new HttpError(415, 'This file does not have a text preview. Download it to open it in another app.');
  if (isWord && size > WORD_PREVIEW_BYTES) throw new HttpError(413, 'Word previews support files up to 20 MiB. You can still download this file.');
  if (isWord && activeWordPreviews >= 2) throw new HttpError(429, 'Document previews are busy. Try again in a moment.');
  if (isWord) activeWordPreviews++;
  try {
    ...
    try {
      ...
      const limit = isWord ? WORD_PREVIEW_BYTES : TEXT_PREVIEW_BYTES;
    ...
    if (isWord) return await wordPreview(bytes, extension);
    let encoding = 'utf-8';
    ...
    throw error;
  } finally { if (isWord) activeWordPreviews--; }
}

function wordPreview(bytes, extension) {
  return new Promise((resolve, reject) => {
    ...
    worker.once('error', () => finish(new HttpError(422, 'This Word document could not be previewed. It may be encrypted, damaged, or too complex.')));
    worker.once('exit', code => { if (!settled) finish(new HttpError(422, `This Word document could not be previewed${code ? ' within the preview limits' : ''}.`)); });
  });
}
